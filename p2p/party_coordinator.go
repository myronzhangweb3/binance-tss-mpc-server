...
func (pc *PartyCoordinator) joinPartyMember(msgID string, peerGroup *peerStatus, sigChan chan string) ([]peer.ID, error) {
	...
	// we trust the returned nodes returned by the leader, if tsshttp fail, the leader
...
func (pc *PartyCoordinator) joinPartyLeader(msgID string, peerGroup *peerStatus, sigChan chan string) ([]peer.ID, error) {
	...
	// if a nodes is not in the list, it means he is not selected by the leader to run the tsshttp
	pc.logger.Debug().Msgf("sending success response to %d all peers", len(allPeers))
	pc.sendResponseToAll(&msg, allPeers)
	return onlinePeers, nil
}

func (pc *PartyCoordinator) JoinPartyWithLeader(msgID string, leaderSalt int64, peers []string, threshold int, sigChan chan string) ([]peer.ID, string, error) {
	leader, err := LeaderNode(msgID, leaderSalt, peers)
	if err != nil {
		return nil, "", err
	}
