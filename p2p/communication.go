...
// TSSProtocolID protocol id used for tsshttp
var (
	TSSProtocolID        protocol.ID = "/p2p/tsshttp"
...
func (c *Communication) handleStreamTss(stream network.Stream) {
	...
	c.logger.Debug().Msgf("reading from tsshttp stream of peer: %s", peerID)
...
func (c *Communication) startChannel(privKeyBytes []byte) error {
	...
	c.host = h
	address, err := conversion.GetPubKeyFromPeerID(h.ID().String())
	if err != nil {
		return fmt.Errorf("fail to GetPubKeyFromPeerID: %w", err)
	}
	c.logger.Info().Msgf("Host created, we are: %s, at: %s, address: %s", h.ID(), h.Addrs(), address)
