...
// PartyInfo the information used by tsshttp key gen and key sign
...
type TssCommon struct {
	...
	P2PPeers                    []peer.ID // most of tsshttp message are broadcast, we store the peers ID to avoid iterating
	msgID                       string
...
func (t *TssCommon) doTssJob(tssJobChan chan *tssJob, jobWg *sync.WaitGroup) {
	...
	for tssjob := range tssJobChan {
		...
		if err != nil {
			t.logger.Error().Err(err).Msg("broken tsshttp share")
			continue
		}
		...
		if errUp != nil {
			...
			t.logger.Error().Err(err).Msgf("fail to apply the share to tsshttp")
			continue
		}
		// we need to retrieve the partylist again as others may update it once we process apply tsshttp share
...
func (t *TssCommon) sendBulkMsg(wiredMsgType string, tssMsgType messages.THORChainTSSMessageType, wiredMsgList []BulkWireMsg) error {
	...
	if err != nil {
		return fmt.Errorf("fail to convert tsshttp msg to wire bytes: %w", err)
	}
...
func (t *TssCommon) ProcessOutCh(msg btss.Message, msgType messages.THORChainTSSMessageType) error {
	...
	// if we cannot get the wire share, the tsshttp will fail, we just quit.
	if err != nil {
