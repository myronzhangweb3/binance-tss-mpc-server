...
func NewTss(
	...
	tssServer := TssServer{
		conf:              conf,
		logger:            log.With().Str("module", "tsshttp").Logger(),
...
func (t *TssServer) Start() error {
	t.logger.Info().Msg("starting the tsshttp servers")
	return nil
}
...
func (t *TssServer) Stop() {
	...
	t.logger.Info().Msg("The tsshttp and p2p server has been stopped successfully")
}
